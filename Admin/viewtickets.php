...
$stmt = $conn->prepare("SELECT * FROM ticket WHERE status='New' OR status='Assigned' OR status='Resolved'"); 
$stmt->execute(); 
...
<body>
    <table border="1">
        ...
        for ($i = 0; $i < $len; $i++) { 
            echo "<tr>";
            echo "<td><a href='assignform.php?ticketid=" . $arr_ticketid[$i] . "'>" . $arr_ticketid[$i] . "</a></td>"; 
